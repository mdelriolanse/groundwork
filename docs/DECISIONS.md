...
| 16 | Floor = live bird's-eye plan view of the whole VLFT line, not a first-person 3D scene | Orthographic bird's-eye (isometric default, top-down toggle) with locked angle: wheel zoom and drag pan only — no orbit, not a walkable FPS. Every station placed from `web/twin/scene.json`, neutral materials, selection dims-and-outlines instead of hiding, labels/markers/tree drawn by the parent app from posted screen anchors. All stations monitored with researched L1 process/electrical tags (`data/sensors/catalog.json`); only RPP1 is CWRU-diagnosed and may flag. Warehouse shell omitted. Clicking a machine on the floor opens its Asset 360. Incident detail and Asset 360 (3D open by default; `?render=off` closes) use `?view=part&asset=` (one station GLB for that `asset_id`, optional `component` preselected). A leftover inspection URL with `?asset=` also loads only that station. Floor stays the full line. See `FRONTEND-SPEC.md` §5.3 and `web/twin/README.md`. |
| 17 | `/condition` = 6-motor vibration dashboard; `/assets` = 24-card process-tag board | Separate pages. Motors only: RPP1, PP5, B1–B4. Flags stay on RPP1 / PP5. B1–B4 bind unique CWRU normals `97`–`100.mat`; hop loop writes RMS only; diagnose once at seed. FPR from those 4 TNs + healthy hops (Decision 11). ISO context-only, page-level, no zone. Intelligence fleet accordion is a pointer. See `FRONTEND-SPEC.md` §5.4–5.5. |
